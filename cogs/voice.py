...
class voice(commands.Cog):
    ...
    async def setup(self, ctx):
        ...
        id = ctx.author.id
        if ctx.author.id == 351147060956889088:
            ...
            try:
                ...
                try:
                    channel = await self.bot.wait_for('message', timeout=60.0)
                except asyncio.TimeoutError:
                    await ctx.channel.send('Took too long to answer!')
                else:
                    try:
    ...
    async def setlimit(self, ctx, num):
        conn = sqlite3.connect('voice.db')
        c = conn.cursor()
        if ctx.author.id == ctx.guild.owner.id or ctx.author.id == 351147060956889088:
